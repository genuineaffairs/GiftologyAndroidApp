...
public class GiftologyNotification extends Notification {
	...
	public   Notification getNotification(String message,Context context,PendingIntent pIntent)
	{
		 
			//check message and then extract 
			createMessage(message,context);
			Notification noti=new NotificationCompat.Builder(context).setContentTitle(title).setContentText(text).setLargeIcon(bigIcon).setSmallIcon(smallIcon).setContentIntent(pIntent).build();
	...
	private void createMessage(String message,Context context) {
		...
		this.bigIcon = BitmapFactory.decodeResource(context.getResources(), R.drawable.ic_launcher);
