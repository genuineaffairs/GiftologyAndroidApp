...
public class Settings extends Activity {
		...
		logout.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				if (ConnectionUtility
					...
					GAUtility.trackEvent(Settings.this, "Settings", "Log out",
							"Logout", (long) 0);
					new LogoutAsynTask().execute("Start");
				} else {
					Toast.makeText(Settings.this, "Internet not available",
	...
	private class LoadProfile extends AsyncTask<String, Integer, String> {
		...
		protected void onPreExecute() {
			...
			/*
			 * if(height==1184 && width==720){ imageSize=190; } if(height==1280
			 * && width==720){ imageSize=190; } else{ imageSize=190; }
			 */
		...
		protected String doInBackground(String... params) {
			try {
				...
				URL image_value = new URL(HomeScreen.prefs.getString(
						"UserPicUrl", ""));
				bmp = BitmapFactory.decodeStream(image_value.openConnection()
						.getInputStream());
	...
	private class LogoutAsynTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pd = ProgressDialog.show(Settings.this, "", "Logging Out...", true,
					false);
		}
	...
	public static int returnScreen(int width, int height) {
		int sizeValue = 0;
		if ((modValue(320 - width) < 30) && (modValue(480 - height) < 30)) { // for
																				// mdpi(320*480)
			sizeValue = 2;
		}
		if ((modValue(480 - width) < 30) && (modValue(800 - height) < 30)) { // for
																				// hdpi(480*800)
			sizeValue = 3;
		}
		if ((modValue(720 - width) < 30) || (modValue(1280 - height) < 30)) { // for
																				// xhdpi(720*1280)
			sizeValue = 4;
		}
		if ((modValue(600 - width) < 30) || (modValue(1024 - height) < 30)) { // for
																				// large-hdpi(600*1024)
			sizeValue = 5;
		}
		if ((modValue(800 - width) < 30) && (modValue(1280 - height) < 30)) { // for
																				// large-hdpi(600*1024)
			sizeValue = 6;
		}
		if ((modValue(720 - width) < 10) && (modValue(1280 - height) < 10)) { // for
																				// large-hdpi(600*1024)
			sizeValue = 7;
		}
