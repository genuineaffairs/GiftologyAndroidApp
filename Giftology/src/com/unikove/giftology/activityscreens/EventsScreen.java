...
public class EventsScreen extends Activity implements OnItemClickListener,
		Runnable {
	...
	RelativeLayout today_lin, tomorrow_lin, this_week_lin, this_month_lin,
			next_month_lin;
	...
	Typeface tf;
	public static String personClickedName, personClickedPicURL,
			personClickedID, flagScreen, personClickedDOB, personClickedTime;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * if(HomeScreen.fromBackPress){ HomeScreen.fromBackPress=false;
		 * finish(); }
		 */
		...
		if (true) {
			...
			startTimer();
			pd = ProgressDialog.show(this, "", "Loading Friends...", true,
					false);
			Thread thread = new Thread(this);
			thread.start();
		}
		/*
		 * else{ setContentView(R.layout.nointernetscreen);
		 * Toast.makeText(EventsScreen.this, "Internet not available",
	 ...
	 * 
	 * Intent alarmGiftIntent = new Intent(EventsScreen.this,
	 * NotificationReciever.class); //alarmGiftIntent.putExtra("userId",
	 * HomeScreen.prefs.getString("userId", null)); PendingIntent
	 * pendingGiftIntent = PendingIntent.getBroadcast(EventsScreen.this, 0,
	 * alarmGiftIntent, PendingIntent.FLAG_UPDATE_CURRENT); AlarmManager
	 * alarmGiftManager = (AlarmManager)getSystemService(Context.ALARM_SERVICE);
	 * 
	 * alarmGiftManager.setRepeating(AlarmManager.RTC,
	 * Calendar.getInstance().getTimeInMillis(),RECEIVERCALLDURATION,
	 * pendingGiftIntent);
	 * 
	 * }
	 */
	...
	/*
	 * public void onRestart(){ super.onRestart(); if(HomeScreen.fromBackPress){
	 * HomeScreen.fromBackPress=false; finish(); } }
	 */
	...
	private class LogoutAsynTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pd = ProgressDialog.show(EventsScreen.this, "", "Logging Out...",
					true, false);
		}
		...
		protected String doInBackground(String... params) {
			try {
				// GAUtility.trackEvent(EventsScreen.this, "Events Screen",
		...
		protected void onPostExecute(String result) {
			if (dataSavedLogout) {
				...
				startActivity(new Intent(EventsScreen.this, HomeScreen.class)
	...
	public void populate_array_list() {
		...
		// dateNow="09/01/2013";
		String today_date = dateNow.substring(0, 2);
		int today_date_int = Integer.parseInt(today_date);
		String today_month = dateNow.substring(3, 5);
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			try {
				if ((today_month_int >= 1) && (today_month_int <= 10)) { // i.e
																			// today_month
																			// is
																			// from
																			// JAN
																			// to
																			// OCT
					String birthdate = jsonArray.getJSONObject(i).getString(
							"birthday_date");
					String month = birthdate.substring(0, 2);
					int month_int = Integer.parseInt(month);
					String date = birthdate.substring(3, 5);
					int date_int = Integer.parseInt(date);

					if ((month_int >= today_month_int)
							&& (month_int <= (today_month_int + 1))) { // i.e
																		// for
																		// current
																		// month
																		// and a
																		// month
																		// ahead
						if (month_int == today_month_int) { // adding dates for
															// current month
							...
							c2.set(today_year_int, today_month_int - 1,
									today_date_int);
							c2.setFirstDayOfWeek(Calendar.MONDAY);
							int crnt_week_number = c2
									.get(Calendar.WEEK_OF_MONTH);

							if (date_int >= today_date_int) { // i.e now we are
																// looking for
																// birthday
																// ahead from
																// today
								// add_in_array_list(i);
								if (date_int == today_date_int) { // selecting
																	// today
																	// birthdays
									status = "today";
									add_in_array_list(i);
								} else if (date_int == (today_date_int + 1)) { // selecting
																				// tomorrow
																				// birthdays
									status = "tomorrow";
									add_in_array_list(i);
								} else if (crnt_week_number == weekNo) { // selecting
																			// birthdays
																			// of
																			// this
																			// week
									status = "this_week";
									add_in_array_list(i);
								} else { // selecting remaining birthdays of
											// this month
									status = "this_month";
									add_in_array_list(i);
								}
					...
					// Log.i("ABC","DEF");
				} else { // i.e now we are looking for when today_month is
							// either NOV or DEC
					String birthdate = jsonArray.getJSONObject(i).getString(
							"birthday_date");
					String month = birthdate.substring(0, 2);
					...
					if (today_month.equals("11")) { // i.e now we are looking
													// for when today_month is
													// NOV
						if ((month.equals("11")) || (month.equals("12"))) {
							if (month.equals("11")) {
								...
								if (date_int >= today_date_int) { // i.e now we
																	// are
																	// looking
																	// for
																	// birthday
						...
						if ((month.equals("12")) || (month.equals("01"))) {
							if (month.equals("12")) { // adding birthdays of
														// December
							...
							if (month.equals("01")) { // adding birthdays of
														// January
	...
	public void add_in_array_list(int i) {
		...
		try {
			...
			name_al.add(jsonArray.getJSONObject(i).getString("name"));
			fd.setDOB(jsonArray.getJSONObject(i).getString("birthday_date"));
			dob_al.add(jsonArray.getJSONObject(i).getString("birthday_date"));
			...
			if (status.equals("today")) {
				if (dec_or_jan.equals("")) {
					al_today.add(fd);
					dob_al_today.add(jsonArray.getJSONObject(i).getString(
							"birthday_date"));
					status = "";
				} else {
			...
			if (status.equals("tomorrow")) {
				if (dec_or_jan.equals("")) {
					al_tomorrow.add(fd);
					dob_al_tomorrow.add(jsonArray.getJSONObject(i).getString(
							"birthday_date"));
					status = "";
				} else {
					if (dec_or_jan.equals("dec")) {
						...
						dec_or_jan = "";
						dob_al_tomorrow.add(jsonArray.getJSONObject(i)
			...
			if (status.equals("this_week")) {
				if (dec_or_jan.equals("")) {
					al_this_week.add(fd);
					dob_al_this_week.add(jsonArray.getJSONObject(i).getString(
							"birthday_date"));
					status = "";
				} else {
					if (dec_or_jan.equals("dec")) {
						...
						dec_or_jan = "";
						dob_al_this_week.add(jsonArray.getJSONObject(i)
								.getString("birthday_date"));
						status = "";
					} else {
			...
			if (status.equals("this_month")) {
				if (dec_or_jan.equals("")) {
					...
					dob_al_this_month.add(jsonArray.getJSONObject(i).getString(
							"birthday_date"));
					status = "";
				} else {
					if (dec_or_jan.equals("dec")) {
						...
						dec_or_jan = "";
						dob_al_this_month.add(jsonArray.getJSONObject(i)
								.getString("birthday_date"));
						status = "";
					} else {
			...
			if (status.equals("next_month")) {
				if (dec_or_jan.equals("")) {
					...
					dob_al_next_month.add(jsonArray.getJSONObject(i).getString(
							"birthday_date"));
					status = "";
				} else {
					if (dec_or_jan.equals("dec")) {
						...
						dec_or_jan = "";
						dob_al_next_month.add(jsonArray.getJSONObject(i)
								.getString("birthday_date"));
						status = "";
					} else {
	...
	public void post_populate_array_list() {
		...
		if (!(al_this_month.size() == 0)) {
			...
			// this_month.setAdapter(new FriendListAdapter(this,
			// al_next_month));
		} else {
		...
		if (!(al_next_month.size() == 0)) {
			...
			// next_month.setAdapter(new FriendListAdapter(this,
			// al_this_month));
		} else {
							...
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog,
									...
									params.putString("picture",
											Utility.HACK_ICON_URL);
									params.putString("name",
											"I am using the Hackbook for Android");
									Utility.mFacebook.dialog(EventsScreen.this,
											"feed", params,
											new PostDialogListener());
								}
							}).setNegativeButton("NO", null).show();
		} catch (JSONException e) {
			Toast.makeText(EventsScreen.this, "Error: " + e.getMessage(),
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	public class PostDialogListener extends BaseDialogListener {
		...
		public void onComplete(Bundle values) {
			...
			if (postId != null) {
				Toast.makeText(EventsScreen.this, "Messase posted on the wall",
						Toast.LENGTH_SHORT).show();
			} else {
				Toast.makeText(EventsScreen.this,
						"Messase not posted on the wall", Toast.LENGTH_SHORT)
						.show();
			}
		}
	}
	...
	public class FriendListAdapter1 extends BaseAdapter {
		...
		public FriendListAdapter1(EventsScreen id,
				ArrayList<FriendDetail> al_fda) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				holder.info = (TextView) hView
						.findViewById(R.id.grid_item_info);
				holder.cal_date = (TextView) hView.findViewById(R.id.cal);
				holder.image_container = (RelativeLayout) hView
						.findViewById(R.id.image);
				holder.desc_container = (LinearLayout) hView
						.findViewById(R.id.desc);
				holder.cakeImage = (RelativeLayout) hView
						.findViewById(R.id.overlapImageCake);
				holder.calImage = (RelativeLayout) hView
						.findViewById(R.id.overlapImageCal);
				hView.setTag(holder);
			}
			...
			if (date_arr.length == 3) {
				...
				int diff = Integer.parseInt(crnt_year)
			...
			hView.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					if (ConnectionUtility
						...
						GAUtility.trackEvent(EventsScreen.this,
								"Events Screen",
								"Todays friend selected for gifting",
								"Friend Selected", (long) 0);

						startActivity(new Intent(EventsScreen.this,
								GiftsToSend.class));
	...
	public class FriendListAdapter2 extends BaseAdapter {
		...
		public FriendListAdapter2(EventsScreen id,
				ArrayList<FriendDetail> al_fda) {
			this.eventsscreen = id;
			this.al_adap = al_fda;
			if (Utility.model == null) {
			...
			hView.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					if (ConnectionUtility
						...
						GAUtility.trackEvent(EventsScreen.this,
								"Events Screen",
								"Tomorrows friend selected for gifting",
								"Friend Selected", (long) 0);
	...
	public class FriendListAdapter3 extends BaseAdapter {
		...
		public FriendListAdapter3(EventsScreen id,
				ArrayList<FriendDetail> al_fda) {
			this.eventsscreen = id;
			this.al_adap = al_fda;
			if (Utility.model == null) {
			...
			hView.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					if (ConnectionUtility
						...
						GAUtility.trackEvent(EventsScreen.this,
								"Events Screen",
								"This weeks friend selected for gifting",
								"Friend Selected", (long) 0);
	...
	public class FriendListAdapter4 extends BaseAdapter {
		...
		public FriendListAdapter4(EventsScreen id,
				ArrayList<FriendDetail> al_fda) {
			this.eventsscreen = id;
			this.al_adap = al_fda;
			if (Utility.model == null) {
			...
			hView.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					if (ConnectionUtility
						...
						GAUtility.trackEvent(EventsScreen.this,
								"Events Screen",
								"This month friend selected for gifting",
								"Friend Selected", (long) 0);
	...
	public class FriendListAdapter5 extends BaseAdapter {
		...
		public FriendListAdapter5(EventsScreen id,
				ArrayList<FriendDetail> al_fda) {
			this.eventsscreen = id;
			this.al_adap = al_fda;
			if (Utility.model == null) {
			...
			hView.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					if (ConnectionUtility
						...
						GAUtility.trackEvent(EventsScreen.this,
								"Events Screen",
								"Next Month friend selected for gifting",
								"Friend Selected", (long) 0);
	...
	public void initializeui() {
		...
		heading1 = (TextView) findViewById(R.id.headingText1);
		heading1.setTypeface(tf);
		heading2 = (TextView) findViewById(R.id.headingText2);
		heading2.setTypeface(tf);
		heading3 = (TextView) findViewById(R.id.headingText3);
		heading3.setTypeface(tf);
		heading4 = (TextView) findViewById(R.id.headingText4);
		heading4.setTypeface(tf);
		heading5 = (TextView) findViewById(R.id.headingText5);
		heading5.setTypeface(tf);
		 ...
		 * public void onClick(View v){ if(popup.getVisibility()==View.GONE){
	...
	private class AboutUSAsync extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... params) {
			String result = "";
			GAUtility.trackEvent(EventsScreen.this, "Event Screen", "About Us",
					"Menu", (long) 0);
			if (HomeScreen.aboutUsText.equals("")) {
				try {
					/*
					 ...
					 * 
					 * HttpEntity entity1 = response.getEntity(); // If the
					 * response does not enclose an entity, there is no need if
					 * (entity1 != null) { InputStream instream =
	...
	private class RegisterUserTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			// pd = ProgressDialog.show(EventsScreen.this, "",
			// "Registering User...");
		}
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				// Log.i("Giftology.Debug","In registeruser 2" );
				jsonUser.put("username", jObject.getString("name"));// Vaibhav
																	// Rastogi
				...
				jsonUserProfile.put("first_name",
						jObject.getString("first_name"));// Vaibhav
				jsonUserProfile
						.put("last_name", jObject.getString("last_name"));// Rastogi
				jsonUserProfile.put("email", "");// null
				jsonUserProfile.put("mobile", "");// null
				try {
					jsonUserProfile.put("city",
							getCityName(jObject.getString("location")));
				} catch (Exception e) {
					jsonUserProfile.put("city", "");
				}
				...
				// Log.i("Giftology.Debug","In registeruser 3" );
				if (!(apiResponse.equals(""))) {
					jsonArray = new JSONArray(apiResponse);
				}
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					...
					String bday = jsonArray.getJSONObject(i).getString(
							"birthday_date");
					String userarrbday[] = bday.split("/");
					if (userarrbday.length == 3) {
						userbdayReverse = userarrbday[2] + "-" + userarrbday[1]
								+ "-" + userarrbday[0];
					...
					jsonFriend.put("friend_fb_id", jsonArray.getJSONObject(i)
							.getLong("uid"));// 100001483700756
					jsonFriend.put("friend_name", jsonArray.getJSONObject(i)
							.getString("name")); // Sarvesh Kumar
				...
				// Log.i("Giftology.Debug","In registeruser 4" );
				json.put("Reminders", jsonArrayReminder);

				/*
				 * Object jresonse1=json.get("User"); Object
				 * jresonse2=json.get("UserProfile"); Object
				 * jresonse3=json.get("UserUtm"); Object
				 * jresonse4=json.get("Reminders");
				 */

				/*
				 * HttpParams httpParams = new BasicHttpParams();
				 * HttpConnectionParams.setConnectionTimeout(httpParams,
				 * 300000); HttpConnectionParams.setSoTimeout(httpParams,
				 * 300000); HttpClient client = new
				 * DefaultHttpClient(httpParams);
				 * 
				 * getHttpEntity
				 * 
				 * HttpPost request = new HttpPost(KeyGenerator.encodedURL(
				 * "http://master.mygiftology.net/users/ws_add.json?"));
				 * request.setEntity(new
				 * ByteArrayEntity(json.toString().getBytes("UTF8")));
				 * HttpResponse response = client.execute(request);
				 */
				// Log.i("Giftology.Debug","In registeruser 5 " );
				String url = GiftologyUtility.GIFTOLOGYSERVER
						+ "users/ws_add.json?";
				HttpEntity inputEntity = (HttpEntity) new ByteArrayEntity(json
						.toString().getBytes("UTF8"));
				// Log.i("Giftology.Debug","In registeruser 6 " );

				org.apache.http.HttpEntity entity1 = new ConnectionUtility().getHttpEntityForPost(url, inputEntity);

				// Log.i("Giftology.Debug","In registeruser" +
				// entity1.toString());
				// If the response does not enclose an entity, there is no need
				if (entity1 != null) {

					str = new ConnectionUtility().parseEntity(entity1).toString();
					if (str.contains("Status\":\"OK")) {
						...
						outer: for (int i = 0; i < arr.length; i++) {
							if (arr[i].equals("user_id")) {
								...
								SharedPreferences.Editor editor = HomeScreen.prefs
										.edit();
								 ...
								 * run() {
								 * Toast.makeText(EventsScreen.this,"USERID = "
		...
		protected void onPostExecute(String result) {
			super.onPostExecute(result);
			/*
			 * pd.dismiss(); if(HomeScreen.prefs.getBoolean("isUserRegistered",
			 * true)){ Toast.makeText(EventsScreen.this,"User Registered...",
			 * Toast.LENGTH_SHORT).show(); } else{
			 * Toast.makeText(EventsScreen.this
			 * ,"User Not Registered.Try next time...",
			 * Toast.LENGTH_SHORT).show(); }
			 */
		}
	}
	...
	public String getCityName(String location) {// "id":"112369278780092","name":"Ghaziabad, India"
		String cityName = "";
		if (location != null) {
			location = location.replace("\"", ""); // id:112369278780092,name:Ghaziabad,
													// India
		...
		TimerTask task = new TimerTask() {
			public void run() {
				...
				List<ActivityManager.RunningTaskInfo> taskInfo = am
						.getRunningTasks(1);
				String currentactivity = taskInfo.get(0).topActivity
						.getClassName();
				if (currentactivity
						.equals("com.unikove.giftology.activityscreens.EventsScreen")) {
					if (progressRunning) {
						if (pd != null) {
							pd.dismiss();
							runOnUiThread(new Runnable() {
								...
								public void run() {
									timeOut = true;
									/*
									 * Utility.mFacebook = new
									 * Facebook(HomeScreen.APP_ID);
									 * 
									 * Log.i("FACEBOOK.DEBUG","Facebook: "+
									 * Utility.mFacebook.toString());
									 * 
									 * restoreCredentials(Utility.mFacebook) ;
									 */
									/*
									 * Utility.mFacebook = new
									 * Facebook(HomeScreen.APP_ID); // Create
									 * the Facebook Object using the app id.
									 * Utility.mAsyncRunner = new
									 * AsyncFacebookRunner(Utility.mFacebook);
