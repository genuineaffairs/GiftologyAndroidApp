...
import com.unikove.giftology.HomeScreen;
...
public class NotificationReciever extends BroadcastReceiver {
	...
	private static final String NOTIFICATIONFLAG="NotificationFlag";
	...
	public boolean restoreCredentials(Facebook facebook,Context context) {
		...
		SharedPreferences sharedPreferences = context.getApplicationContext().getSharedPreferences(KEY, Context.MODE_PRIVATE);
...
@Override
	...
	public void onReceive(Context arg0, Intent arg1) {
		...
		notificationPreferences=arg0.getApplicationContext().getSharedPreferences(NKEY, Context.MODE_PRIVATE);
		if(notificationPreferences!=null)
		{
			BIRTHDAYGIFTNOTGIVEN=notificationPreferences.getBoolean(BIRTHDAYGIFTNOTGIVENKEY, true);
		
		}
		
		if(currentDate.getTime().getHours()==0 && currentDate.getTime().getMinutes()==4)
		{
			BIRTHDAYGIFTNOTGIVEN=true;
			Editor editor = arg0.getApplicationContext().getSharedPreferences(NKEY, Context.MODE_PRIVATE).edit();
			editor.putBoolean(BIRTHDAYGIFTNOTGIVENKEY, true);
			editor.commit();
		}
		
		if(currentDate.getTime().getHours()==18 && currentDate.getTime().getMinutes()==0)
		{
			BIRTHDAYGIFTNOTGIVEN=true;
			Editor editor = arg0.getApplicationContext().getSharedPreferences(NKEY, Context.MODE_PRIVATE).edit();
			editor.putBoolean(BIRTHDAYGIFTNOTGIVENKEY, true);
		...
		if (isInternetAvailable(arg0)) {
			...
			/*if (countForEvent == DURATIONFOREVENTCHECK) {
				...
				if (!(message.equalsIgnoreCase(lastEventMessage))) {
					if (!message.equalsIgnoreCase(EMPTYSTRING)) {
						lastEventMessage = message;
						String sendmessage = message + BIRTHDAY;
	...
	private String runCommand(Context context,String commandType)
	{
		...
		if(HomeScreen.prefs==null){
			...
			HomeScreen.prefs = context.getApplicationContext().getSharedPreferences("PREF_NAME", Context.MODE_PRIVATE);
		...
		else if(commandType.equalsIgnoreCase(FRIEND))
		{
			...
			NewFriendCommand newFriendCommand = new NewFriendCommand(restConnect);
		...
		}else 
		{
			...
			NewCelebrationCommand newCelebrationCommand = new NewCelebrationCommand(restConnect);
	...
	private void makeNotifications(String message,Context context)
	{
		try {
			...
			// Build notification by calling GiftologyNotification (builder)class
			
			Notification noti = new GiftologyNotification().getNotification(message,context,pIntent);
