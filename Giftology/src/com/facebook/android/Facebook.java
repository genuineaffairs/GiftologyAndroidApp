...
public class Facebook {
	...
	/**
	 * Retrieve the last time the token was updated (in milliseconds since the
	 * Unix epoch), or 0 if the token has not been set.
	 * 
	...
	/**
	 * Restore the token, expiration time, and last update time from cached
	 * values. These should be values obtained from getAccessToken(),
	 * getAccessExpires, and getLastAccessUpdate() respectively.
	 * 
	 * @param accessToken
	 *            - access token
	 * @param accessExpires
	 *            - access token expiration time
	 * @param lastAccessUpdate
	 *            - timestamp of the last token update
	 */
	public void setTokenFromCache(String accessToken, long accessExpires,
	 ...
	 * @return Attribution ID that will be used for conversion tracking. It will
	 *         be null only if the user has not installed or logged in to the
	 *         Facebook app.
	 */
	...
	/**
	 * Get the auto install publish setting. If true, an install event will be
	 * published during authorize(), unless it has occurred previously or the
	 * app does not have install attribution enabled on the application's
	 ...
	 * @throws Exception
	 */
	private static void publishInstall(final Facebook fb,
			final String applicationId, final Context context)
			throws JSONException, FacebookError, MalformedURLException,
			IOException {

		String attributionId = Facebook.getAttributionId(context
				.getContentResolver());
		SharedPreferences preferences = context.getSharedPreferences(
				ATTRIBUTION_PREFERENCES, Context.MODE_PRIVATE);
		String pingKey = applicationId + "ping";
		long lastPing = preferences.getLong(pingKey, 0);
		if (lastPing == 0 && attributionId != null) {
			Bundle supportsAttributionParams = new Bundle();
			supportsAttributionParams.putString(APPLICATION_FIELDS,
					SUPPORTS_ATTRIBUTION);
			JSONObject supportResponse = Util.parseJson(fb.request(
					applicationId, supportsAttributionParams));
			Object doesSupportAttribution = (Boolean) supportResponse
					.get(SUPPORTS_ATTRIBUTION);
			...
			if ((Boolean) doesSupportAttribution) {
				...
				String publishUrl = String.format(PUBLISH_ACTIVITY_PATH,
						applicationId);
	...
	void autoPublishAsync(final Context context) {
		...
		synchronized (this) {
			if (mAutoPublishAsyncTask == null) {
				...
				if (applicationId != null) {
					asyncTask = mAutoPublishAsyncTask = new AutoPublishAsyncTask(
	...
	private class AutoPublishAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... voids) {
			try {
				Facebook.publishInstall(Facebook.this, mApplicationId,
						mApplicationContext);
			} catch (Exception e) {
				Util.logd("Facebook-publish", e.getMessage());
			}
